<%@ Page Language="VB" MasterPageFile="~/MasterPage2.master" AutoEventWireup="false" CodeFile="Default2.aspx.vb" Inherits="Default6" title="租賃中心 - 租圈" %>
        ...
        <EmptyItemTemplate>
            ...
            <ItemTemplate>
                <td id="Td3" runat="server" style="">
                    ...
                    <asp:HyperLink ID="HyperLink1" runat="server" NavigateUrl='<%# "Default2.aspx?layer=" & Eval("ca_id") %>' Text='<%#Eval("ca_name")%>' Font-Size="80%" Font-Bold="True"></asp:HyperLink>
                    </div>
                </td>
            </ItemTemplate>
...
<asp:SqlDataSource ID="SqlDataSource4" runat="server" 
        ...
        SelectCommand="SELECT top 4 * FROM [commodity] WHERE whether_shelf = 0 AND ( merchandise = 1 OR merchandise = 2)  AND com_id like @layer + '%' ORDER BY RAND()">
    <SelectParameters>
...
<br />
    ...
    <div style="border :2px solid #000000; height: 1.7em; background-image:url(images/bg_header.gif); color:White ; padding-top:2px; width :578px; margin:0px 16px; font-weight :bold ;">搜尋結果</div>
        <asp:SqlDataSource ID="SqlDataSource3" runat="server" 
            ConnectionString="<%$ ConnectionStrings:DB_RentConnectionString2 %>" 
            SelectCommand="SELECT * FROM [commodity], [member_data] WHERE commodity.re_member_id = member_data.id AND whether_shelf = 0 AND ([com_id] LIKE @Param1 + '%') AND com_name like '%' + @search + '%' AND state like @state AND area like @area AND rent >= @rent1 AND rent <= @rent2 AND (merchandise = 1 OR merchandise = 2) AND com_name like '%' + @search2 + '%'">
            <SelectParameters>
                ...
                <ItemTemplate>
                    <td id="Td1" runat="server" style="">
                        ...
                        <li>評價：<%#Eval("evaluat")%></li>
                         ...
                         <div style="width:280px; margin-left:10px;"><span>訂金：<%#Eval("add_money")%>元</span>｜<span>租金：<%#Eval("rent")%>元</span>｜<span>租期：<%#Eval("days")%>天</span></div>
                        </div>
                    </td>
                </ItemTemplate>
